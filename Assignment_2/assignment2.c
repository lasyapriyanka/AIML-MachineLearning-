...
void factor(double **A, double **B,double **C, int a, int b, int c,double factor){
	for (int i=0;i<a;i++){
		for (int k=0;k<c;k++){
			double s = 0.0;
			for (int j=0;j<b;j++){
...
double RandomNumber(float Minimum, float Maximum)
{
...
void check_classification(double **output,double **target_feature_data, int rows,int classes,char c){
	...
	for (int i=0;i<rows;i++){
		...
		printf("Case(%d): Original_class : (%d)----classified_as : (%d)\n",i,original,index);
	}
	accuracy = (accuracy * 100) / ((double) rows);
	printf("Classified Correct = %d\n Classified Wrong = %d\n",correct,rows-correct);
	if (c=='r')
		printf("Accuracy of Training(in percentage) = %.4f\n",accuracy);
	else if (c=='e')
		printf("Accuracy of Testing(in percentage) = %.4f\n",accuracy);
}

double error_func(double **output, double **target_feature_data, int rows){
...
int i=0,j=0,rows,columns;
	...
	factor(test_feature_dupdata,w_ih,test_hidden,rows,columns,n+1,1.0);
	for (int i=0;i<rows;i++){
		test_hidden[i][n]=1.00; 
		for (int j=0;j<n;j++){
			test_hidden[i][j] = sigmoid(test_hidden[i][j]);
		}
	}

	factor(test_hidden,w_ho,test_output,rows,n+1,3,1.0);
...
double **test_target_data; 
	...
	for (int row=0;row<ARR_SIZE;row++){
		target_feature_data[row] = (double *) malloc(3 * sizeof(double)); 
	}
	...
	while(getline(&line,&count, file)!=-1){
		...
		while(pt != NULL){
			...
			pt = strtok(NULL, ",");
			if (pt == NULL)
				target_feature_data[i][(int)a - 1] = 1.00; 
	...
	int n;
	printf("Input Value(Maximum) = %f\nInput Value(Minimum) = %f\n",max_x_value,min_x_value);
...
factor(train_feature_dupdata,w_ih,hidden,rows,ARR_SIZE1,n+1,1.0);
...
factor(hidden,w_ho,output,rows,n+1,3,1.0);
...
double error_value,error_threshold = 1.0,eta = 0.1;
error_value = error_func(output,target_feature_data,rows);
...
while (error_value > error_threshold){
	...
	factor(hidden_transpose,delta_k,delta_w_ho,n+1,rows,3,eta);
//end of w_ho weights calculation
	mat_transpose(delta_k,delta_k_transpose,rows,3);
	factor(w_ho,delta_k_transpose,temp_w_del,n+1,3,rows,1.0);
	for(int i=0;i<n+1;i++){
	...
	factor(temp_w_del,train_feature_dupdata,delta_w_ih_transpose,n+1,rows,ARR_SIZE1,eta);
